...
namespace ApiPortal.Controllers
{
    ...
    public class AutomatizacionController : ControllerBase
    {
        ...
        public AutomatizacionController(PortalClientesSoftlandContext context, IWebHostEnvironment webHostEnvironment, PortalAdministracionSoftlandContext admin, IHttpContextAccessor contextAccessor)
        {
            _context = context;
        ...
        [HttpGet("EnviaAutomatizacionesMultiplesTenants"), Authorize]
        public async Task<ActionResult<object>> EnviaAutomatizacionesMultiplesTenants()
        {
            try
            {
                int horaActual = DateTime.Now.Hour;
                ...
                foreach (var tenant in tenants)
                {
                    ...
                    if (parametroEjecuta != null)
                    {
                        if (parametroEjecuta.Valor == "1")
                        {
                            ...
                            LogApi logApi = new LogApi();
                            logApi.Api = "api/Automatizacion/EnviaAutomatizacionesMultiplesTenants";
                            logApi.Inicio = DateTime.Now;
                            logApi.Id = RandomPassword.GenerateRandomText() + logApi.Inicio.ToString();
                            ...
                            try
                            {
                                Generador genera = new Generador(newContextPortal, _webHostEnvironment);
                                ...
                                #region COBRANZA CLASICA
                                //Genera envio de cobranzas, la ejecución la realiza algun procedimiento externo
                                var listaAutomatizaciones = newContextPortal.Automatizacions.Where(x => x.Estado == 1).ToList();
                                var feriados = newContextPortal.Feriados.ToList();
                                var clientesExcluidos = newContextPortal.ClientesExcluidos.ToList();
                                var configPago = newContextPortal.ConfiguracionPagoClientes.FirstOrDefault();
                                int correosEnviados = 0;
                                //Recorremos una a una las cobranzas
                                foreach (var automatizacion in listaAutomatizaciones)
                                {
                                    ...
                                    var horario = newContextPortal.CobranzaHorarios.Where(x => x.IdHorario == automatizacion.IdHorario).FirstOrDefault();
                                    if (horario.Hora != horaActual)
                                    {
                                        continue;
                                    }
                                    ...
                                    MailService mailService = new MailService(newContextPortal, _webHostEnvironment);
                                    int idEstadoEnvio = await mailService.EnviaAutomatizacionAsync(listaEnvio, automatizacion);
                                    if (idEstadoEnvio != 0) { IdEstadoFinal = idEstadoEnvio; }
